...
async def get_user_history(
    ...
    start_date: str = None, # Expects YYYY-MM-DD format from frontend
):
    """Fetches analysis history for the current user with optional filtering."""
    
    user_id = current_user['id']
    query = analysis_results.select().where(analysis_results.c.user_id == user_id)
    
    # 1. Idea Summary Search (If search_query is provided)
    if search_query:
        query = query.where(analysis_results.c.idea_summary.ilike(f"%{search_query}%"))

    # 2. Date Filter (If start_date is provided)
    if start_date:
        try:
            # Convert start_date string to datetime object (Start of the day)
            dt_start = datetime.datetime.strptime(start_date, "%Y-%m-%d")
            dt_end = dt_start + datetime.timedelta(days=1)
            
            # Filter where created_at is between start_date (00:00:00) and end_date (00:00:00 next day)
            query = query.where(analysis_results.c.created_at >= dt_start).where(analysis_results.c.created_at < dt_end)
            
        except ValueError:
            # If the date format is wrong, raise an error
            raise HTTPException(status_code=400, detail="Invalid date format. Use YYYY-MM-DD.")


    # Apply ordering and limit
    query = query.order_by(analysis_results.c.created_at.desc()).limit(10)
    
    history = await database.fetch_all(query)
    
    formatted_history = [
        {
            # Date format includes both date and time 
